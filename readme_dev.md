...
The architecture follows a **Model-View-Controller (MVC)** pattern. To maintain performance while handling 3D volumetric datasets, the application separates the heavy immutable physical data from the transient UI viewing state, and uses "dirty flags" to ensure DearPyGui only updates what has actually changed.
...
* **`VolumeData` (The Physical Data):** The immutable source of truth for a loaded image.
    * Manages the SimpleITK image and the zero-copy numpy array (`self.data`).
    * Stores physical metadata (spacing, origin, matrix, pixel type).
    * Handles 3D spatial math (mapping 3D voxels to physical millimeters).
    * *Note:* This class is completely unaware of how it is being viewed.


* **`ViewState` (The UI State):** Stores all transient viewing parameters.
    * Tied to a specific `VolumeData` object.
    * Stores Window/Level, camera zoom/pan, current slice depths, and visibility toggles.
    * Tracks the crosshair voxel and physical coordinates.
    * *Dirty Flag:* `is_data_dirty` (True when W/L or slice depth changes).


* **`SliceRenderer` (The Workhorse):** A stateless utility class.
    * Takes a `VolumeData` and a `ViewState`, slices the raw 3D array, applies the Window/Level transfer function, and safely packs it into a flat 1D RGBA array for the GPU.


* **`Controller` (The Manager):** Acts as the central nervous system.
    * Maintains the dictionaries of all loaded `VolumeData`s, `ViewState`s, and `SliceViewer`s.
...
* **The Render Loop (`run()`):** The beating heart of the application. It runs every frame, checking the "dirty flags" of the `ViewState` and `SliceViewer` to determine if a texture needs to be recalculated or a UI element needs to be synced.
...
* **`SliceViewer`:** Represents a single viewport (e.g., "V1").
    * Pulls physical data from `self.volume` and UI parameters from `self.view_state`.
...
* **`file_dialog.py` & `resources.py`:** Platform-agnostic utilities for native OS file browsers and loading bundled assets (like fonts and icons).
...
2. **`is_data_dirty`:**
    ...
    * *Action:* Forces the `SliceRenderer` to slice the 3D numpy array, apply the W/L, and push the new 1D RGBA array to the GPU texture registry.
...
* **Adding a Math Filter:** Add the calculation to `VolumeData` in `core.py`, and set `view_state.is_data_dirty = True`.
