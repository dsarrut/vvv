...
from .ui_landmark import LandmarkPluginUI
from .control_landmark import LandmarkPluginController


class LandmarkPlugin(PluginProtocol):
    plugin_id = "landmark_plugin"
    label = "Landmarks"
    description = "Manage 3D physical point landmarks."
    order = 22

    def __init__(self):
        self._controller = LandmarkPluginController(self.plugin_id)
        self._ui = LandmarkPluginUI(self.plugin_id, self._controller)
