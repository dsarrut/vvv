...
def _validate_and_reshape(
    ...
    If that still fails, logs a warning and returns (None, None).
    Otherwise returns (reshaped_2d, actual_shape).
    """
    flat_len = len(flat_data)
    expected_len = expected_shape[0] * expected_shape[1] * 4
    ...
    if flat_len != expected_len:
        if (
            ...
            logging.warning(
                f"Shape mismatch in {context_name}: flat size is {flat_len}, "
                f"but expected {expected_len} (shape {expected_shape}). Skipping."
            )
            return None, None
...
class TextureManager:
    ...
    def _upload_sw_nn_base(self, vs, canvas_w, canvas_h):
        ...
        rgba_2d, actual_shape = _validate_and_reshape(
            v.last_rgba_flat,
            actual_shape,
            v.get_slice_shape(),
            "base texture",
    ...
    def _apply_merged_overlay_blend(
        ...
        if (
            ...
            ov_rgba_2d, ov_actual_shape = _validate_and_reshape(
                v.last_overlay_rgba_flat,
                ov_actual_shape,
                v.get_slice_shape(),
                "overlay merged blend",
            )
            if ov_rgba_2d is None:
            ...
            if show_slider:
                ...
                if dpg.does_item_exist(slider_zoom_tag):
                    ...
                    if current_s >= max_val - 0.5:
                        max_val = float(np.ceil(current_s + 1.0))
                        dpg.configure_item(slider_zoom_tag, max_value=max_val)
                    elif current_s <= min_val + 0.5:
                        min_val = float(np.floor(current_s - 1.0))
                        dpg.configure_item(slider_zoom_tag, min_value=min_val)

                    # Contract range back towards default [-3.0, 5.0] if far inside
                    if current_s < max_val - 2.0 and max_val > 5.0:
                        max_val = max(5.0, float(np.ceil(current_s + 1.0)))
                        dpg.configure_item(slider_zoom_tag, max_value=max_val)
                    if current_s > min_val + 2.0 and min_val < -3.0:
                        min_val = min(-3.0, float(np.floor(current_s - 1.0)))
                        dpg.configure_item(slider_zoom_tag, min_value=min_val)
