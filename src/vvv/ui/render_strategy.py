...
def compute_native_voxel_base(viewer, pmin, pmax, canvas_w, canvas_h):
    """Fast interactive rotation preview: render the base image via affine canvas mapping.

    During a rotation drag (vs._is_interactive_rotation = True), this replaces the
    slow ITK resample path. Canvas pixels are mapped directly to the base image's
    native voxel space through the inverse of the active registration transform.
    The result is a canvas-sized RGBA flat array ready for texture upload.

    Analogous to compute_native_voxel_overlay but for the base image itself.
    """
    vs = viewer.view_state
    if not vs or not vs.space.is_active or not vs.space.has_rotation():
        return None

    base_vol = viewer.volume
    if not base_vol or base_vol.data is None:
        return None
    ...
    # Inverse base transform via safe numpy inversion
    xfm = vs.space.get_rotation_only_transform() if (vs.space.transform and vs.space.is_active) else None
    R, t = _affine_np(xfm)
    try:
    ...
    M_base = base_vol.matrix * base_vol.spacing[np.newaxis, :]
    M_inv_base = base_vol.inverse_matrix / base_vol.spacing[:, None]

    # canvas pixel → base physical (M_base) → apply inverse transform (R_inv, t_inv)
    # → base physical again → base voxel (M_inv_base)
    A_total = M_inv_base @ R_inv @ M_base
    b_total = M_inv_base @ (R_inv @ base_vol.origin + t_inv - base_vol.origin)

    slice_h, slice_w = viewer.get_slice_shape()
    depth = float(viewer.slice_idx)

    time_idx = min(vs.camera.time_idx, base_vol.num_timepoints - 1)
    base_data = base_vol.data[time_idx] if base_vol.num_timepoints > 1 else base_vol.data
    b_D, b_H, b_W = base_data.shape

    # Screen crop optimization (same as overlay version)
    c_x0, c_x1, c_y0, c_y1 = 0, canvas_w, 0, canvas_h
    try:
        A_inv = np.linalg.inv(A_total)
        corners = np.array([
            [0, 0, 0], [b_W, 0, 0], [0, b_H, 0], [b_W, b_H, 0],
            [0, 0, b_D], [b_W, 0, b_D], [0, b_H, b_D], [b_W, b_H, b_D],
        ])
        base_corners = (A_inv @ (corners - b_total).T).T

        if viewer.orientation == ViewMode.AXIAL:
            ix_c = base_corners[:, 0] + 0.5
            iy_c = base_corners[:, 1] + 0.5
        elif viewer.orientation == ViewMode.SAGITTAL:
            ix_c = slice_w - base_corners[:, 1] - 0.5
            iy_c = slice_h - base_corners[:, 2] - 0.5
        else:
            ix_c = base_corners[:, 0] + 0.5
            iy_c = slice_h - base_corners[:, 2] - 0.5

        xs = (ix_c * disp_w / slice_w) + pmin[0] - 0.5
        ys = (iy_c * disp_h / slice_h) + pmin[1] - 0.5
        c_x0 = max(0, min(canvas_w, int(np.floor(xs.min()))))
        c_x1 = max(0, min(canvas_w, int(np.ceil(xs.max())) + 1))
        c_y0 = max(0, min(canvas_h, int(np.floor(ys.min()))))
        c_y1 = max(0, min(canvas_h, int(np.ceil(ys.max())) + 1))
    except np.linalg.LinAlgError:
        pass

    if not hasattr(viewer, "_fast_base_buf") or viewer._fast_base_buf.shape[:2] != (canvas_h, canvas_w):
        viewer._fast_base_buf = np.zeros((canvas_h, canvas_w, 4), dtype=np.float32)
        viewer._last_fast_base_crop = None

    rgba = viewer._fast_base_buf
    if viewer._last_fast_base_crop:
        oy0, oy1, ox0, ox1 = viewer._last_fast_base_crop
        rgba[oy0:oy1, ox0:ox1] = 0.0
    viewer._last_fast_base_crop = (c_y0, c_y1, c_x0, c_x1)
    ...
    if use_numba:
        thr_nb = np.float32(-np.inf)  # no threshold for base image
        wl_min = np.float32(vs.display.wl - vs.display.ww * 0.5)
        wl_scale = np.float32(1.0 / max(vs.display.ww, 1e-20))
        base_arr = np.ascontiguousarray(base_data)
        ...
        _native_ov_kernel_nb(
            ...
            lut, wl_min, wl_scale, thr_nb, np.float32(1.0),
            c_y0, c_x0, b_D, b_H, b_W,
            False,  # precomposite=False: overwrite (standalone base, no alpha blend)
        )
        return rgba.ravel()
    ...
    in_bounds = (s_x >= 0) & (s_x < b_W) & (s_y >= 0) & (s_y < b_H) & (s_z >= 0) & (s_z < b_D)
    if in_bounds.any():
        x_nn = s_x[in_bounds].astype(np.int32)
        y_nn = s_y[in_bounds].astype(np.int32)
        z_nn = s_z[in_bounds].astype(np.int32)
        vals = base_data[z_nn, y_nn, x_nn].astype(np.float32)
        norm = SliceRenderer.normalize_wl(vals, vs.display.ww, vs.display.wl)
        colors = lut[(norm * 255).astype(np.uint8)]
        rgba_crop = rgba[c_y0:c_y1, c_x0:c_x1]
        rgba_crop[in_bounds] = colors
