...
def select_nn_mode(cfg: dict, has_fusion: bool) -> NNMode:
    ...
    is_single = (
        has_fusion if st_cfg == "Auto" else (st_cfg is True or st_cfg == "Single")
    )
...
def should_use_lazy_lin(
    cfg: dict, has_fusion: bool, is_hw: bool, use_numba: bool = False
) -> bool:
...
def blend_slices_cpu(base_2d, ov_2d, opacity, shift_x, shift_y):
    ...
    out[y0_base:y1_base, x0_base:x1_base, :3] = (
        o_roi[..., :3] * alpha_ov + b_roi[..., :3] * inv_alpha
    )
...
def compute_software_nearest_neighbor(
    ...
    ix_full = np.floor(
        (np.arange(canvas_w, dtype=np.float32) + 0.5 - pmin[0]) * (w / disp_w) + 1e-5
    ).astype(np.int32)
    iy_full = np.floor(
        (np.arange(canvas_h, dtype=np.float32) + 0.5 - pmin[1]) * (h / disp_h) + 1e-5
    ).astype(np.int32)
...
def compute_native_voxel_overlay(
    ...
    base_xfm = (
        vs.space.transform if (vs.space.transform and vs.space.is_active) else None
    )
    ov_xfm = (
    ...
    if use_numba:
        ...
        _native_ov_kernel_nb(  # type: ignore[misc]
            ...
            C_col.astype(np.float32),
            C_row.astype(np.float32),
            iy_adj.astype(np.float32),
            ix_adj.astype(np.float32),
            lut,
            wl_min,
            wl_scale,
            thr_nb,
    ...
    if in_bounds.any():
        x_nn, y_nn, z_nn = (
            s_x[in_bounds].astype(np.int32),
            s_y[in_bounds].astype(np.int32),
            s_z[in_bounds].astype(np.int32),
        )
        ...
        if in_bounds.any():
            ...
            if target_buffer is not None:
                ...
                rgba_crop[in_bounds, :3] = (
                    new_colors[:, :3] * alpha + dst_colors[:, :3] * inv_alpha
                )
...
def compute_overlay_preview_2d_affine(
    base_vol, overlay_vol, orientation, slice_idx, R, center, time_idx
):
    ...
    if orientation == ViewMode.AXIAL:
        ...
        rows, cols = np.meshgrid(
            np.arange(shape[0]), np.arange(shape[2]), indexing="ij"
        )
        vox_N = np.column_stack(
            [
    ...
    vox_ov_N = (
        (phys_ov_N - overlay_vol.origin) @ overlay_vol.inverse_matrix.T
    ) / overlay_vol.spacing
