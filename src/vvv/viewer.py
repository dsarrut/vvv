...
class SliceViewer:
    ...
    @property
    def slice_idx(self):
        return self.view_state.slices[self.orientation] if self.view_state else None
    ...
    @property
    def pan_offset(self):
        return self.view_state.pan[self.orientation] if self.view_state else [0, 0]
    ...
    @property
    def zoom(self):
        return self.view_state.zoom if self.view_state else 1.0

    @zoom.setter
    def zoom(self, value):
        if self.view_state: self.view_state.zoom = value
    ...
    def set_orientation(self, orientation):
        ...
        if self.current_image_id:
            ...
            self.set_image(self.current_image_id)
    ...
    def should_use_voxels_strips(self):
        if not self.current_image_model or self.current_image_model.interpolation_linear: return False
        win_w, win_h = dpg.get_item_width(f"win_{self.tag}"), dpg.get_item_height(f"win_{self.tag}")
        if not win_w or not win_h: return False
        pmin, pmax = self.current_pmin, self.current_pmax
        _, shape = self.current_image_model.get_slice_rgba(self.slice_idx, self.orientation)
    ...
    def update_sidebar_window_level(self):
        if self.view_state:
            dpg.set_value("info_window", f"{self.view_state.ww:g}")
            dpg.set_value("info_level", f"{self.view_state.wl:g}")
    ...
    def on_drag(self, data):
        ...
        if not is_ctrl and not is_shift and dpg.is_mouse_button_down(dpg.mvMouseButton_Left):
            ...
            self.update_window_level(max(1, self.view_state.ww + sx * 2), self.view_state.wl - sy * 2)
