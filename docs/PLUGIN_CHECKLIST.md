...
- [ ] **Renamable Text Inputs** — For editable names (e.g. images, ROIs, profiles), use the shared `build_renamable_input` component from `ui_components.py` to ensure names are safely updated on Enter/focus-loss, automatically namespacing and shielding input keys from leaking to shortcuts.
