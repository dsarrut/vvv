...
- **[plugin_registration.py](../src/vvv/plugins/registration/plugin_registration.py)**: Plugin entry point. Defines metadata (exposing the user-facing label `"Transform"`), registers lifecycle events, and delegates to the controller and UI.
- **[control_registration.py](../src/vvv/plugins/registration/control_registration.py)**: Controller. Coordinates transform calculations, background worker threads, auto-resampling timers, file dialogues (load, save, save as), and manual slider adjustments.
- **[ui_registration.py](../src/vvv/plugins/registration/ui_registration.py)**: UI. Renders registration sliders, Center of Rotation (CoR) pivot settings, matrix views, and toggles for fast preview/auto-resample. Handles Beginner Mode UI visibility logic.

---

## 1. Transform State Ownership & Workspace Serialization

Registration state lives in the **core** layout model under `ViewState.space` (an instance of the `SpatialEngine` class defined in [geometry.py](../src/vvv/maths/geometry.py)):

- The actual transform is a `SimpleITK.Euler3DTransform`.
- `vs.space.is_active` flags whether the transform is currently active.
- **Workspace-Only Serialization**: In [control_registration.py](../src/vvv/plugins/registration/control_registration.py), `serialize_image_state` and `restore_image_state` inspect the call stack using Python's `inspect` module:
